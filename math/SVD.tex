...
\text{$\circled{6}$ ~$A=U\Sigma V^T$ separates $A$ into $r$ rank 1 matrices $\sum_{i=1}^{r}\sigma_{i}\mathbf{u}_i\mathbf{v}_i^T$.}\\
\text{$\circled{7}$ ~$A^TA=\sum_{i=1}^{r}\sigma_{i}^2\mathbf{v}_i\mathbf{v}_i^T$  and the right singular vectors of $A$ are eigenvectors of $A^TA$ with eigenvalues $\sigma_{i}^2=\lambda_i$.}\\
\text{$\circled{8}$ ~$AA^T=\sum_{i=1}^{r}\sigma_{i}^2\mathbf{u}_i\mathbf{u}_i^T$  and the left singular vectors of $A$ are eigenvectors of $AA^T$ with eigenvalues $\sigma_{i}^2=\lambda_i$.}\\
